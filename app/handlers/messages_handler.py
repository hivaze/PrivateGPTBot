...
from app.models_api.open_ai_client import create_message, truncate_user_history, count_tokens
...
async def communication_answer(session: Session, message: types.Message,
    ...
    try:  # try-finally block for precise lock release
        ...
        async with TypingBlock(message.chat):
            ...
            history = history[-settings.config.context.last_messages_count:]
            history, removed_tokens = truncate_user_history(tg_user.username, pers_prompt,
                                                            history, previous_tokens_usage)
